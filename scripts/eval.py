...
def check_base():
    ...
    # Отсечка вопросов вне темы: проверяется без модели, поэтому живёт здесь,
    # а не среди эталонных ответов. Реальные случаи из чата: бот отвечал «это
    # вне моей темы» на вопросы про собственный релиз 2.2.0 — про MCP, про
    # коннектор, про версию. Словарь темы собирается из документации, значит
    # ломается он молча и вместе с ней — проверять надо на каждом обновлении.
    import telegram_bot as bot

    ontopic = [
        "появился MCP я так понимаю как его поставить ан мой клауд код",
        "проверь в релизе 2 2 0 коннектор к ЬСЗ",
        "ты сам пишешь режим агента через MCP как его активровать",
        "что нового в версии 2.2.0",
        "как поставить теги на проект",
        "какие роли есть в ФБ",
    ]
    offtopic = [
        "Столица македонии какая?",
        "Расскажи какая погода в нижнем",
        "Сто умножить на 3",
        "кто выиграл чемпионат мира по футболу",
        "как приготовить борщ",
    ]
    add("словарь темы", len(bot.domain_stems()) >= 1000,
        f"{len(bot.domain_stems())} основ из документации")
    # Поиск не задействуем (chunks=[]): проверяем именно словарь, а не порог
    # по расстоянию — иначе проверка потребует эмбеддингов и станет платной.
    cut = [q for q in ontopic if bot.looks_offtopic(q, [])]
    add("вопросы по теме доходят до модели", not cut,
        "все проходят" if not cut else f"отсечены: {cut}")
    passed = [q for q in offtopic if not bot.looks_offtopic(q, [])]
    add("вопросы вне темы отсекаются", not passed,
        "все отсечены" if not passed else f"прошли к модели: {passed}")
