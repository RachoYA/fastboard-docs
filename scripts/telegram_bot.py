...
GREETING = (
    "Здравствуйте! 👋🏻 Добро пожаловать в службу заботы Fastboard.\n\n"
    "Я — Фиби, AI-консультант Fastboard. Помогу разобраться с платформой, "
    "настройкой аналитики и работой с данными.\n\n"
    "Ко мне можно обратиться, если нужно:\n"
    "• создать или настроить дашборд, виджет или визуализацию;\n"
    "• подключить и настроить источник данных;\n"
    "• настроить роли и права доступа;\n"
    "• разобраться с SQL-запросом или ClickHouse;\n"
    "• найти причину ошибки или некорректного отображения данных;\n"
    "• настроить Fastboard или решить вопрос по администрированию;\n"
    "• понять, как лучше реализовать вашу задачу средствами платформы.\n\n"
    "Просто опишите, что хотите сделать или с чем возникла проблема. Технические "
    "термины не обязательны — я помогу сформулировать задачу и предложу следующий шаг 💙\n\n"
    "При необходимости дам пошаговую инструкцию, пример настройки или ссылку на "
    "нужный раздел документации Fastboard."
)
...
SERVICE_PHRASES = [
    (re.compile(r"\bв предоставленн\w+ (?:контексте|материалах|фрагмент\w+|документации|"
                r"таблице(?: свойств)?|справочнике|описании|тексте|выдержке|данных)",
                re.I), "в документации"),
    (re.compile(r"\bв приведённ\w+ (?:фрагмент\w*|контексте)", re.I), "в документации"),
    (re.compile(r"\bв (?:данном|этом|указанном) фрагменте(?: справочника| документации)?",
                re.I), "в документации"),
    (re.compile(r"\b(?:согласно|по) предоставленн\w+ \w+(?: свойств)?", re.I),
     "по документации"),
    (re.compile(r"\bне (?:приведен\w*|указан\w*|перечислен\w*) в таблице(?: свойств)?",
                re.I), "не описано в документации"),
...
HEDGES = [
    re.compile(r"\s*\((?=[^)]{0,90}(?:в зависимости от верси|или аналогичн|обычно|как правило))"
               r"[^)]{0,140}\)", re.I),
    re.compile(r",?\s*(?:или аналогичн\w+ раздел\w*[^.,;]{0,60}|"
               r"в зависимости от верси[^.,;]{0,40})", re.I),
    re.compile(r"\bобычно (?:называется|это|находится|это раздел)\b", re.I),
    re.compile(r"\bкак правило,? (?:это|называется)\b", re.I),
    # «(или Описание, если настраиваете конкретный показатель)» — альтернатива
    # с условием внутри скобок это всегда догадка о названии раздела.
    re.compile(r"\s*\((?=[^)]{0,80}\bесли\b)\s*или [^)]{0,120}\)", re.I),
]
...
STEM = 5
# Сколько статей должны знать слово, чтобы считать его словом темы. Заголовков
# это не касается: там случайных слов нет.
MIN_DOCS = int(os.environ.get("DOMAIN_MIN_DOCS", "5"))
...
def build_domain_stems():
    ...
    Заголовки берём целиком — там нет вёрстки, и одного такого слова довольно.
    Из текста статей берём слова, которые встречаются не меньше чем в пяти
    статьях, но не в каждой четвёртой: повсеместное — это либо служебное слово,
    либо мусор шаблона (icon, svg, uploads), а единичное — случайность. На
    проде «Сто умножить на 3» и «кто выиграл чемпионат мира по футболу»
    доходили до модели из-за слов «умножить» и «мира», встречающихся ровно в
    одной и в трёх статьях из четырёхсот.
    ...
    words = (heads | {w for w, n in body.items() if MIN_DOCS <= n <= popular}) - STOP_WORDS
...
class Rag:
    ...
    def search_all(self, question, top_k=TOP_K, split=True):
        ...
        # Вопрос про цвет, фон или шрифт — это вопрос к таблице свойств, даже
        # если слова «настройка» в нём нет: иначе справочник получает четыре
        # строки из двух десятков и ответ собирается по чужим статьям.
        about_settings = any(word in question.lower() for word in
                             ("json", "свойств", "настройк", "параметр", "settings",
                              "цвет", "фон", "шрифт", "подпис", "стил", "ячейк"))
    ...
    def answer(self, question, history=(), extra_context="", search_query=None, on_delta=None):
        ...
        # В сообщении от лица пользователя — только то, что он прислал на самом
        # деле. Когда найденная документация лежала здесь же, модель считала её
        # присланной клиентом: на скриншот таблицы она отвечала «вы прислали
        # только фрагмент документации по виджету „Таблица“, а не сам файл» —
        # хотя таблица была у неё перед глазами и она её же и описала.
        #
        ...
        if extra_context:
            parts.append(
                "Я прислал вложение (изображение или файл). Вот что в нём "
                "(пересказывать содержимое мне не нужно — я его видел, "
                "используй как условие задачи):\n" + extra_context)
        parts.append(f"Вопрос пользователя (отвечай именно на него): {question}")
        ...
        # Контекст — в конце системного сообщения: он меняется на каждый вопрос,
        # а всё, что до него, остаётся неизменным и по-прежнему берётся из кэша.
        system += ("\n\nКонтекст из документации. Это выдержки, подобранные поиском по "
                   "справке под текущий вопрос: пользователь их не присылал и не видит. "
                   "Никогда не пиши, что клиент прислал документацию или её фрагменты, и "
                   "не путай их с его вложением.\n\n"
        ...
        if doubled_quotes(reply):
            ...
            messages.append({"role": "user", "content":
                             ...
                             "и правило упадёт. Выбери ОДИН вариант и выдержи его во всём "
                             "ответе: либо условие `WHERE region = '{{region}}'` со значением "
                             "`Москва` без кавычек, либо условие "
                             "`WHERE region in ( {{regions}} )` со значением `'Москва'` в "
                             "кавычках. Заголовок и примеры не должны противоречить друг "
                             "другу. Покажи, во что превращается условие после подстановки."})
            reply = ollama_chat(messages)

        bad = unknown_paths(reply, context) if context else []
        if bad:
            # Целиком: обрезка до трёх звеньев заставляла модель считать, что
            # более глубоких имён не существует, и заявлять это клиенту.
            allowed = sorted(set(JSON_PATH.findall(context)))
            if allowed:
                ...
                messages.append({"role": "user", "content":
                                 ...
                                 + "\n".join(allowed[:120])
                                 + "\n\nЭто выдержка из таблицы, а не весь список: "
                                 "не утверждай, что других путей не существует."})
                reply = ollama_chat(messages)

            # Если переспрос не помог, строки с выдуманными путями просто
            # убираются: пусть ответ будет короче, чем отправит клиента искать
            # настройку, которой нет.
            still_bad = unknown_paths(reply, context)
            if still_bad:
                kept = [line for line in reply.splitlines()
                        if not any(bad in line for bad in still_bad)]
                cleaned = "\n".join(kept).strip()
                log.info("убраны строки с выдуманными путями: %s", still_bad[:3])
                if len(cleaned) > 200:
                    reply = cleaned
...
def handle_commands(chat_id, text, msg_from=None):
    ...
    if cmd == "/reset":
        ...
        send(chat_id, "История диалога очищена 🧹 и мы готовы начать с чистого листа!\n\n"
                      "Просто напишите, с чем вам нужна помощь — я здесь и внимательно слушаю.")
        return True
    if cmd == "/status":
        try:
...
HUMAN_PATTERNS = re.compile(
    ...
    r"живой человек|живого человека|техподдержк|тех\.?\s?поддержк|"
    r"(?:нужен|позов\w+|свяж\w+|дайте|соедин\w+|переключ\w+)[^\n]{0,20}оператор|"
    r"(?:пообщать\w*|поговорить|связать\w*|свяжите|соедините)[^\n]{0,40}"
    r"(?:поддержк\w*|специалист\w*|ребят\w*|команд\w*|сотрудник\w*)|"
    r"службу заботы|службой заботы|человека позов|верните меня к человеку|"
    r"если ты не помог|ты не помог|бот не помога)", re.I)


# Модель иногда обещает передать обращение сама, хотя просьба не подошла под
# шаблон и передачи не было. Обещание должно исполняться, иначе клиент ждёт
# ответа, которого не будет.
ESCALATION_PROMISE = re.compile(
    r"(передаю[^\n]{0,30}(обращени|вопрос)|передал[^\n]{0,30}(обращени|вопрос)|"
    r"переда[юмн][^\n]{0,30}специалист)", re.I)
...
def support_contact_text(escalated=False):
    ...
    if escalated:
        return ("Я уже направил ваш вопрос нашему специалисту из Fastboard🧑🏻\u200d💻 "
                "Он скоро подключится к диалогу и ответит прямо здесь.\n\n"
                + (SUPPORT_CONTACT + "\n\n" if SUPPORT_CONTACT else "")
                + "Пока ждёте, я остаюсь на связи — могу помочь найти информацию в "
                "документации или разобраться с другим вопросом.")
    if SUPPORT_CONTACT:
...
def process(batch):
    ...
    with Typing(chat_id):
        ...
        # Если модель пообещала передать обращение, а передачи не было —
        # передаём сейчас, чтобы обещание не осталось словами.
        if ESCALATION_PROMISE.search(answer):
            if escalate(chat_id, question, batch[-1].get("from")):
                log.info("chat=%s обещание передачи исполнено", chat_id)
            else:
                answer += "\n\n" + support_contact_text(False)

        send(chat_id, answer, reply_to=reply_to)
        # Вложение помечаем и в истории — той же фразой, что и в самом вопросе.
        # Без пометки в следующей реплике модель видела просто текст и отвечала
        # «не вижу таблицы» на скриншот, который сама же и разобрала минуту назад.
        stored = question
        if recognized:
            stored += "\n\nЯ прислал вложение. Вот что в нём:\n" + recognized[:2000]
        history.add(chat_id, stored.strip(), answer)
