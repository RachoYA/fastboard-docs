...
UI_NAMES = [
    ...
    # Права на поток делятся на три категории: «Работа с потоком»,
    # «Управление проектом», «Работа с проектом». Право «Просмотр потока»
    # модель придумывает регулярно — клиент будет искать его в интерфейсе зря.
    (re.compile(r"«?\bпросмотр(?:а|у|ом)?\s+потока\b»?", re.I), "«Работа с потоком»"),
]
