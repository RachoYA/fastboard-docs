...
| Юнит | Что делает |
|---|---|
| `fastboard-watchdog.timer` | раз в 5 минут проверяет бота, туннель к GPU, доступность модели и место на диске; при сбое пишет в журнал и шлёт сообщение в `ADMIN_CHAT_ID` (не чаще раза в час) |
| `fastboard-backup.timer` | ежедневно в 04:30 архивирует `.env`, `vectordb/`, `state/` и манифест — то, что не восстанавливается из git; хранит последние `BACKUP_KEEP` копий |

Каталог копий задаётся `BACKUP_DIR`. По умолчанию он локальный: это спасает от
порчи данных, но не от потери сервера — для защиты от неё укажите каталог на
удалённом диске.

Доступ к боту ограничивается `ALLOWED_CHATS` (пусто — отвечает всем) и
`RATE_LIMIT_PER_HOUR`. Токен бота маскируется во всех записях журнала:
`requests` кладёт полный URL в текст исключения, и без маскировки токен
утекает в systemd-журнал открытым текстом.

## Готовность к масштабированию

Оценка по пяти направлениям с измерениями на живой системе — в
[`READINESS.md`](READINESS.md): пропускная способность, конвейер базы,
качество ответов, эксплуатация и контроль качества.
