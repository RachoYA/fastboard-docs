...
Source: https://clickhouse.com/docs/ru/sql-reference/data-types/decimal
...
Проверки на переполнение приводят к замедлению операций. Если известно, что переполнения невозможны, имеет смысл отключить их с помощью настройкиdecimal_check_overflow. Когда проверки отключены и происходит переполнение, результат будет некорректным:
